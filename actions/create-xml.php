...
$storename = Array();
$usename = 0;
$names = mysql_query("SELECT address, address2, city, state, zip, name, lat, lng, phone, fax, url, special FROM $table");
...
if ($usename == 1) {
	$query = "SELECT address, address2, city, state, zip, name, lat, lng, phone, fax, url, special FROM $table WHERE name = '".$storename[0]."'";
	...
	$query = sprintf("SELECT address, address2, city, state, zip, name, lat, lng, phone, fax, url, special, ( 3959 * acos( cos( radians('%s') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('%s') ) + sin( radians('%s') ) * sin( radians( lat ) ) ) ) AS distance FROM $table HAVING distance < '%s' ORDER BY distance LIMIT 0 , 20",
