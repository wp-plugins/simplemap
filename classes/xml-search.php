		...
		function init_search() {
			if ( isset( $_GET['sm-xml-search'] ) ) {
				...
				$sql = $wpdb->prepare( "
					...
					FROM 
						wp_posts as posts, 
						( SELECT lat_pm.meta_value as lat, lat_pm.post_id as lat_id FROM wp_postmeta lat_pm WHERE lat_pm.meta_key = 'location_lat' ) as lat_tbl, 
						( SELECT lng_pm.meta_value as lng, lng_pm.post_id as lng_id FROM wp_postmeta lng_pm WHERE lng_pm.meta_key = 'location_lng' ) as lng_tbl  
					WHERE 
						lat_id = posts.ID 
						AND lng_id = posts.ID 
